...
const SyncImport: FunctionComponent<SyncImportProps> = ({ paginationOptions }) => {
  ...
  const handleFileImport = async (file: File) => {
    if (!file) return;
    try {
      const data = await fetchQuery(syncImportQuery, { file }).toPromise();
      const { synchronizerAddInputFromImport } = data as SyncImportQuery$data;
      setIngestSynchronizerData(synchronizerAddInputFromImport);
      setOpen(true);
