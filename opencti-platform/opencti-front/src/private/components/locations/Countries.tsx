...
import { CountriesPaginationQuery, CountriesPaginationQuery$variables } from './__generated__/CountriesPaginationQuery.graphql';
import { CountriesData_data$data } from './__generated__/CountriesData_data.graphql';
import { CountriesLinesPaginationQuery$variables } from './countries/__generated__/CountriesLinesPaginationQuery.graphql';
...
const countriesQuery = graphql`
  query CountriesPaginationQuery(
    $search: String
    $count: Int!
    $cursor: ID
...
const countriesFragment = graphql`
  fragment CountriesData_data on Query
  @argumentDefinitions(
    search: { type: "String" }
  ...
  @refetchable(queryName: "CountriesRefetchQuery") {
    countries(
      search: $search
      first: $count
      after: $cursor
...
const Countries: FunctionComponent = () => {
  ...
  const { viewStorage, paginationOptions, helpers: storageHelpers } = usePaginationLocalStorage<CountriesPaginationQuery$variables>(
    LOCAL_STORAGE_KEY,
    initialValues,
  );

  const contextFilters = useBuildEntityTypeBasedFilterContext('Country', viewStorage.filters);
  const queryPaginationOptions = {
    ...paginationOptions,
    filters: contextFilters,
  } as unknown as CountriesPaginationQuery$variables;
  const queryRef = useQueryLoading<CountriesPaginationQuery>(
    countriesQuery,
  ...
  const dataColumns: DataTableProps['dataColumns'] = {
    name: {
      ...
      render: (data) => {
        ...
        const link = `${resolveLink('Country')}/${data.id}`;
        return (
          <Tooltip title={value}>
  ...
  const preloadedPaginationOptions = {
    linesQuery: countriesQuery,
    linesFragment: countriesFragment,
    queryRef,
    nodePath: ['countries', 'pageInfo', 'globalCount'],
    setNumberOfElements: storageHelpers.handleSetNumberOfElements,
  } as UsePreloadedPaginationFragment<CountriesPaginationQuery>;

  const createButton = (
    <Security needs={[KNOWLEDGE_KNUPDATE]}>
      <CountryCreation paginationOptions={queryPaginationOptions as unknown as CountriesLinesPaginationQuery$variables} />
    </Security>
  );
        ...
        <Box sx={{ padding: '24px 24px 0 24px' }}>
          ...
          >
            <Box>
              ...
              >
                {t_i18n('Countries tracked in the intelligence workspace')}
              </Typography>
            </Box>
          ...
          {queryRef && (
            <DataTable
              ...
              resolvePath={(data: CountriesData_data$data) => data.countries?.edges?.map((n) => n?.node)}
              ...
              emptyStateMessage={t_i18n('No countries yet. Create one to start tracking countries.')}
            />
          )}
        </Box>
      </ExportContextProvider>
