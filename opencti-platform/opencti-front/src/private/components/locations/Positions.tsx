...
import { PositionsPaginationQuery, PositionsPaginationQuery$variables } from './__generated__/PositionsPaginationQuery.graphql';
import { PositionsData_data$data } from './__generated__/PositionsData_data.graphql';
import { PositionsLinesPaginationQuery$variables } from './positions/__generated__/PositionsLinesPaginationQuery.graphql';
...
const positionsQuery = graphql`
  query PositionsPaginationQuery(
    $search: String
    $count: Int!
    $cursor: ID
...
const positionsFragment = graphql`
  fragment PositionsData_data on Query
  @argumentDefinitions(
    search: { type: "String" }
  ...
  @refetchable(queryName: "PositionsRefetchQuery") {
    positions(
      search: $search
      first: $count
      after: $cursor
...
const Positions: FunctionComponent = () => {
  ...
  const { viewStorage, paginationOptions, helpers: storageHelpers } = usePaginationLocalStorage<PositionsPaginationQuery$variables>(
    LOCAL_STORAGE_KEY,
    initialValues,
  );

  const contextFilters = useBuildEntityTypeBasedFilterContext('Position', viewStorage.filters);
  const queryPaginationOptions = {
    ...paginationOptions,
    filters: contextFilters,
  } as unknown as PositionsPaginationQuery$variables;
  const queryRef = useQueryLoading<PositionsPaginationQuery>(
    positionsQuery,
  ...
  const dataColumns: DataTableProps['dataColumns'] = {
    name: {
      ...
      render: (data) => {
        ...
        const link = `${resolveLink('Position')}/${data.id}`;
        return (
          <Tooltip title={value}>
  ...
  const preloadedPaginationOptions = {
    linesQuery: positionsQuery,
    linesFragment: positionsFragment,
    queryRef,
    nodePath: ['positions', 'pageInfo', 'globalCount'],
    setNumberOfElements: storageHelpers.handleSetNumberOfElements,
  } as UsePreloadedPaginationFragment<PositionsPaginationQuery>;

  const createButton = (
    <Security needs={[KNOWLEDGE_KNUPDATE]}>
      <PositionCreation paginationOptions={queryPaginationOptions as unknown as PositionsLinesPaginationQuery$variables} />
    </Security>
  );
        ...
        <Box sx={{ padding: '24px 24px 0 24px' }}>
          ...
          >
            <Box>
              ...
              >
                {t_i18n('Positions and roles tracked in the intelligence workspace')}
              </Typography>
            </Box>
          ...
          {queryRef && (
            <DataTable
              ...
              resolvePath={(data: PositionsData_data$data) => data.positions?.edges?.map((n) => n?.node)}
              ...
              emptyStateMessage={t_i18n('No positions yet. Create one to start tracking positions and roles.')}
            />
          )}
        </Box>
      </ExportContextProvider>
